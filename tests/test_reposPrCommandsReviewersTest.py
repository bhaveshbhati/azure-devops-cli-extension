...
class AzReposPrTests(DevopsScenarioTest):
    ...
    def test_pull_request_createUpdateVoteListAbandonReactivateCompleteReviewers(self):
        ...
        try:
            ...
            create_repo_command = f'az repos create --detect false --name {random_repo_name} --project {PROJECT_NAME} --output json'
            ...
            import_repo_command = f'az repos import create --git-source-url https://dev.azure.com/devops-cli-test-org/TestSupportProject/_git/snakes-and-ladders --repository {created_repo_id} --project {PROJECT_NAME} --detect false --output json'
            ...
            create_pr_command = f'az repos pr create -p {PROJECT_NAME} -r {created_repo_id} -s testbranch -t main --title "{pr_title}" -d "Sample PR description" --detect false --output json'
            ...
            update_pr_command = f'az repos pr update --id {create_pr_id} -d "{updated_description}" --detect false --output json'
            ...
            # List PR
            pr_list_output = self.cmd(f'az repos pr list -p {PROJECT_NAME} -r {created_repo_id} --detect false --output json', checks=[
                self.check("[0].description", updated_description)
            ]).get_output_in_json()
            assert len(pr_list_output) > 0

            # Show PR 
            show_pr_command = f'az repos pr show --id {create_pr_id} --detect false --output json'
            ...
            # Abandon PR
            abandon_pr_command = f'az repos pr update --id {create_pr_id} --detect false --output json --status abandoned'
            ...
            # Reactivate PR 
            reactivate_pr_command = f'az repos pr update --status active --id {create_pr_id} --detect false --output json'
            ...
            # Add PR reviewer
            add_pr_reviewers_command = f'az repos pr reviewer add --id {create_pr_id} --reviewers "bhaveshbhati90@outlook.com" --detect false --output json'
            add_pr_reviewers_output = self.cmd(add_pr_reviewers_command).get_output_in_json()
            assert len(add_pr_reviewers_output) > 0

            # List PR reviewer
            list_pr_reviewers_command = f'az repos pr reviewer list --id {create_pr_id} --detect false --output json'
            list_pr_reviewers_output = self.cmd(list_pr_reviewers_command).get_output_in_json()
            assert len(list_pr_reviewers_output) > 0

            # Remove PR reviewer
            remove_pr_reviewers_command = f'az repos pr reviewer remove --id {create_pr_id} --reviewers "bhaveshbhati90@outlook.com" --detect false --output json'
            ...
            # Vote on PR 
            vote_pr_command = f'az repos pr set-vote --id {create_pr_id} --vote approve --detect false --output json'
            ...
            # Complete PR 
            complete_pr_command = f'az repos pr update --status completed --id {create_pr_id} --detect false --output json'
            ...
            delete_repo_command = f'az repos delete --detect false --id {created_repo_id} --project {PROJECT_NAME} -y --output json'
            self.cmd(delete_repo_command)
            
            # Verify Deletion
            list_repo_command = f'az repos list --project {PROJECT_NAME} --output json --detect false'
